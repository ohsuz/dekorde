...
class Transformer(nn.Module):
    def __init__(
        ...
        # --- layers to optimise --- #
        self.token_embeddings = nn.Embedding(num_embeddings=vocab_size, embedding_dim=hidden_size)
        self.pos_embeddings = nn.Embedding(num_embeddings=max_length, embedding_dim=hidden_size)
        self.encoder = Encoder(hidden_size, max_length, heads, num_layers, dropout)  # the encoder stack
        self.decoder = Decoder(hidden_size, max_length, heads, num_layers, dropout, lookahead_mask)  # the decoder stack
    ...
    def inference(self, X: torch.Tensor, Y: torch.Tensor) -> torch.Tensor:
        """
        A function for computing the loss for this batch.
        :param X: (N, L) - source
        :param Y: (N, 2, L) - target. (N, 0, L) = Y_l.  (N, 1, L) = Y_r.
        ...
        W_hy = self.token_embeddings.weight  # (|V|, H)
        logits = torch.einsum("abc,dc->adb", H_y, W_hy)  # (N, |V|, L)
        max_indices = torch.argmax(logits, dim=1)  # (N, L)
        return max_indices
